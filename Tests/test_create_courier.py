...
class TestCreateCourier:
    ...
    @allure.description('Проверяем успешное создание курьера и ответный статус 201 + тело ответа')
    ...
    def test_courier_same_name(self, default_courier):
        ...
        response = requests.post(urls.URL_BASE + urls.URL_CREATE_COURIER, data = payload)
        assert response.status_code == 409 and response.json()["message"] == TestDataBody.courier_wrong_login_409_text
    ...
    def test_courier_without_login(self):
        response = requests.post(urls.URL_BASE + urls.URL_CREATE_COURIER)
        assert response.status_code == 400 and response.json()["message"] == TestDataBody.courier_wrong_login_400_text
